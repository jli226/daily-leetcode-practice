...
def removeKthLinkedListNode(head, k):
    ...
    # 这个部分我不是很懂这个写法？ 我的理解目的是要让fast先跑出去和slow保持距离
    for i in range(k):
        if not fast:  # 这个fast不是一个pointer吗？为什么可以直接写not in fast这样
            return head
        fast = fast.next
